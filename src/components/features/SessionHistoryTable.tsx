...
export const SessionHistoryTable: React.FC<SessionHistoryTableProps> = ({ room }) => {
  ...
  return (
    <div style={{ overflowX: 'auto', padding: '8px' }}>
      <table
        style={{ width: '100%', borderCollapse: 'collapse', minWidth: '300px', fontSize: '14px' }}
      >
        <thead>
              ...
              {players.map((p) => {
                ...
                return (
                  <td key={p.id} style={{ padding: '8px', textAlign: 'center' }}>
                    ...
                    >
                      {pt > 0 ? '+' : pt < 0 ? '-' : ''}
                      {Math.abs(pt).toLocaleString(undefined, {
          ...
          <tr style={{ borderTop: '2px solid #444', backgroundColor: 'rgba(255,255,255,0.05)' }}>
            ...
            {players.map((p) => {
              ...
              return (
                <td
                  key={p.id}
                  style={{ padding: '12px 8px', textAlign: 'center', fontWeight: 'bold' }}
                >
                  <span
                    style={{
