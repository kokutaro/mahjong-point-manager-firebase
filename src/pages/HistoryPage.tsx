  ...
  return (
    <div style={{ padding: '16px', maxWidth: '600px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h2>対戦履歴</h2>
        <Button size="small" variant="secondary" onClick={() => navigate('/')}>トップへ</Button>
      </div>

      {rooms.length === 0 ? (
        <div style={{ textAlign: 'center', color: '#aaa', marginTop: '40px' }}>履歴はありません</div>
      ) : (
          ...
          {rooms.map(room => (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                <span style={{ fontWeight: 'bold' }}>{room.settings.mode === '3ma' ? '3人打ち' : '4人打ち'}</span>
              ...
              <div style={{ marginTop: '8px', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '8px' }}>
                <span style={{ fontSize: '0.8em', color: room.status === 'ended' ? '#aaa' : '#4caf50' }}>
