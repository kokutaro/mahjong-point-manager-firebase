...
- **部屋作成**: 新しく対局ルームを作成します。
- **ユーザー設定**: 表示名、アバター、部屋作成時・大会作成時のデフォルト設定を保存します。
- **対戦履歴**: 過去の対局履歴を確認できます。
- **部屋ID入力**: 友人が作成した部屋に参加する場合、ここに部屋IDを入力して参加します（同期モード時）。

## 2.1 ユーザー設定

トップ画面の「ユーザー設定」から、以下の内容を UID ごとに保存できます。

- **表示名**: 部屋作成や大会作成時の初期表示名として利用されます。
- **アバター**: 現時点では設定画面で保存のみ行います。
- **部屋作成デフォルト**: 部屋作成モーダルを開いたときの初期ルールとして利用されます。
- **大会作成デフォルト**: 大会作成画面を開いたときの初期ルールとして利用されます。

Firestore に設定が存在しない場合でも、従来どおりブラウザ保存の表示名や既定ルールで利用できます。
...
- **SE**: 対局画面の SE スイッチを ON にすると、リーチ、ロン、ツモのタイミングで効果音が再生されます。対局中でもいつでも ON/OFF を切り替えできます。
