...
- `useFuCalculation = false` の場合、1〜3翻は部屋設定の固定点数を使用する。
- 初期値は 1翻: 子1000/親1500, 2翻: 子2000/親3000, 3翻: 子4000/親6000。
- 4翻以上は引き続き満貫以上として扱う。
- 固定点数は部屋作成時に 100 点刻みで変更できる。
- ツモ支払いは設定されたロン点から通常の切り上げ式で算出し、3麻では欠ける 1 人分を既存ロジックどおり按分する。
...
- **親の和了時**: 本場は維持されず、加算される（連荘扱い）。
  - _Correction_: 親のアガリでも本場数字自体は増える（1本場→2本場）。「解消」されるのは「誰かが（親以外が、または親が上がっても局が変わる場合）」という概念だが、正確には「局遷移時に0になるか、+1になるか」で定義済。
...
- 設定 `yakitoriEnabled = true` の場合のみ適用する。
- 焼き鳥判定はその対局の `HandLog` 全体で行い、`result.type === 'Win'` の `winners[].id` を和了者集合とする。
- 和了者集合に含まれないプレイヤーを焼き鳥対象とし、和了者1人あたり `yakitoriPoint` 点を支払う。
- 精算は終局時に1回のみ適用する（局ごとの都度精算はしない）。
- 全員未和了（和了者集合が空）の場合は焼き鳥精算を行わない。
